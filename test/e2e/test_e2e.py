...
def test_default_enabled_state():
    bus = FakeBus()
    skill = HomeAssistantSkill(bus=bus, skill_id="homeassistant_skill.test")
...
def test_intent_disable_setting():
    bus = FakeBus()
    skill = HomeAssistantSkill(bus=bus, skill_id="homeassistant_skill.test", settings={"disable_intents": True})
